...
class Search:
    ...
    def cve_search(self, cve_search_str: str) -> str:
        ...
        if os.path.isabs(cve_search_path):
            cve_search_bin = os.path.join(os.path.realpath(cve_search_path), cve_bin)
        else:
            cve_search_bin = os.path.join(os.path.dirname(os.path.realpath(__file__)), cve_search_path, cve_bin)
